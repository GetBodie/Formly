...
import type { StorageClient, SyncResult, DownloadResult, FolderInfo, StorageFile } from './types'
import { DocumentTooLargeError, MAX_FILE_SIZE } from './types'
...
export const dropboxClient: StorageClient = {
  ...
  async downloadFile(fileId: string): Promise<DownloadResult> {
    ...
    const fileBlob = (response.result as { fileBinary: Buffer }).fileBinary
